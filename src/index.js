...
import {
    ...
    convertActualClausesToLatex, convertClausesToLatex, updateResolutionTable, initControlButtons, returnDivsWithExplanation,
    setWhichSelectToPaste, generateLatexTable
} from './ui'
import {updateTranslations } from './change-language.js';

const changeLanguageSelect = document.getElementById('changeLanguage');
...
changeLanguageSelect.addEventListener('change', function() {
    ...
    const selectedLanguage = changeLanguageSelect.value;
    const clausesElementText = document.getElementsByClassName('clauses-element-text');
    const clausesElementsArray = Array.from(clausesElementText);
    const button = document.getElementById('toggleVisibilityButton');


    function translateExplanation() {
        if (resultOfResolutionMethod !== null) {
            showResultOfResolutionMethod(resultOfResolutionMethod);
            MathJax.typesetPromise();
        }

        const inputFormula = model.getValue();
        const lines = inputFormula.split('\n').filter(line => line.trim() !== '');
        const explanation = returnDivsWithExplanation();

        if (inputFormula.includes("⊢")) {
            changeLogicalSyllogismExplanationLanguage(explanation);
        } else if (lines.length === 1) {
            changeLanguageOfFormulaExplanation(explanation);
        } else if (lines.length >= 2) {
            changeLogicalSyllogismExplanationLanguage(explanation);
        }
    }

    if (selectedLanguage === 'anglictina') {
        ...
        whichLanguage = 'en';
        if(!isAnySteps) {
            resolutionContainer.textContent = 'No resolution steps have been taken, no complementary literals have been found.';
        }
        if(isProvingStarted && clausesElementText !== null) {
            clausesElementText.innerHTML = 'Actual clauses:';
            const newText = 'Actual clauses:';

            clausesElementsArray.forEach(element => {
        ...
        if(isProvingStarted && clausesElementText !== null) {
            ...
            const newText = 'Aktuálne klauzuly:';
...
let resultOfResolutionMethod =  null;
let whichLanguage = 'sk';
...
export function applyResolution() {
    if (resolutionFound) {
        if(whichLanguage === 'sk') {
            ...
            displayMessage("A solution has already been found. Further use of the resolution rule is not possible.", 'warning');
        }
        return;
    }
    ...
    if (clause1 === clause2) {
        if(whichLanguage === 'sk') {
            ...
            displayMessage("You chose the same clauses. Select different clauses.", 'warning');
        }
        return;
    }
    ...
    if (usedPairs.has(pairKey)) {
        if(whichLanguage === 'sk') {
            ...
            displayMessage("This pair of clauses has already been used. Select another pair.", 'warning');
        }
        return;
    }
    ...
    if (result) {
        setWhichSelectToPaste(1);
        setIfRoot(true);
        clearTree("#dynamic-tree-container");
        clearTableResolutionInterpretation();

        function addUniqueResult(clausesForEach, result) {
            const resultExists = clausesForEach.some(clause => {
                if (Array.isArray(clause) && Array.isArray(result)) {
                    return clause.length === result.length && clause.every((val, index) => val === result[index]);
                } else {
        ...
        if (result.length === 0) {
            if(whichLanguage === 'sk') {
                ...
                displayMessage("Empty clause reached. The formula is proven!", 'warning');
            }
...
function parseDIMACS(content) {
    ...
    for (let line of lines) {
        ...
        if (terms[terms.length - 1] !== '0') {
            if(whichLanguage === 'sk') {
                ...
                throw new Error('Each clause must be ended with a null.');
            }
        }
    ...
    if (!processingClauses) {
        if(whichLanguage === 'sk') {
            throw new Error('The file does not contain a valid "p cnf" header.');
        } else if(whichLanguage === 'en') {
            throw new Error('Each clause must be ended with a null.');
        }
    }
    if (!hasValidClauses) {
        if(whichLanguage === 'sk') {
            ...
            throw new Error('The file does not contain any valid clauses.');
        }
    }
...
function showResultOfResolutionMethod(result) {
    ...
    function generateResult(formulaLaTeX) {
        if (whichLanguage === 'sk') {
            ...
            resolutionResultElement.innerHTML = result.isProved ?
                "Formula \\(" + formulaLaTeX + "\\)  <strong style='color: #09866B'>is proved</strong>." :
                "Formula \\(" + formulaLaTeX + "\\) <strong style='color: #861809'>isn't proved (satisfiable)</strong>.";
        }
    }

    if (lines.some(line => line.includes('⊢'))) {
        const formulaLaTeX = convertFormulasToLatex(relevantLine);
        generateResult(formulaLaTeX)
    } else if (lines.length === 1) {
        ...
        if(whichLanguage === 'sk') {
            ...
            resolutionResultElement.innerHTML = result.isProved
                ? "Formula " + logicalConsequence.map(cnf => "\\((" + convertFormulasToLatex(cnf.toString()) + "\\))").join(' \\( \\wedge \\) ') + " <strong style='color: #09866B'>is proved</strong>."
                : "Formula " + logicalConsequence.map(cnf => "\\((" + convertFormulasToLatex(cnf.toString()) + "\\))").join(' \\( \\wedge \\)') + " <strong style='color: #861809'>isn't proved (satisfiable)</strong>.";
        }
...
function resolutionExplanation(clauses) {
    ...
    resultOfResolutionMethod = result;
    ...
    function produceResolutionSteps() {
        ...
        result.steps.forEach((step, index) => {
            ...
            const clausesElementText = document.createElement('span');
            if(whichLanguage === 'sk') {
                clausesElementText.innerHTML = 'Aktuálne klauzuly:';
            } else if(whichLanguage === 'en') {
                clausesElementText.innerHTML = 'Actual clauses:';

            }
            clausesElementText.className = 'clauses-element-text';
            clausesElementText.style.marginBottom = '5px';
            stepElement.appendChild(clausesElementText);
            const latexClauses = convertActualClausesToLatex(step.clauses);
            clausesElement.innerHTML = `${latexClauses}`;
...
function undo() {
    if (undoStack.length === 0) {
        if(whichLanguage === 'sk') {
            ...
            displayMessage("It is not possible to take a step back.", 'error');
        }
        return;
    }
...
function redo() {
    if (redoStack.length === 0) {
        if(whichLanguage === 'sk') {
            ...
            displayMessage("It is not possible to take a step forward.", 'error');
        }
        return;
    }
...
function headersOfLogicalSyllogismExplanation(explanation) {
    if (whichLanguage === 'sk') {
        ...
        explanation.descFormula.innerHTML = "<strong>1. step is </strong>the transformation of assumptions into <strong>CNF</strong> (assumptions are in conjunction): ";
        explanation.descCNF.innerHTML = "<strong>2. step</strong> is the <strong>negation of the conclusion</strong> and its transformation into <strong>CNF</strong>: ";
        explanation.descClauses.innerHTML = "<strong>3. step is</strong> to apply <strong>the resolution rule</strong> to the individual clauses: ";
    }
}

function changeLogicalSyllogismExplanationLanguage(explanation) {
    headersOfLogicalSyllogismExplanation(explanation);
...
function startProving() {
    ...
    if (inputFormula !== lastInputTry || isNegationChecked !== lastIsChecked || wasStrategyChanged) {
        ...
        //         if (modeRadios[i].value === newValue) {
...
function changeLanguageOfFormulaExplanation(explanation) {
    if(whichLanguage === 'sk') {
        explanation.descFormula.innerHTML = "<strong>1. krok</strong> je <strong>negácia</strong> formuly: ";
    } else if(whichLanguage === 'en') {
        explanation.descFormula.innerHTML = "<strong>1. step</strong> is <strong>negation</strong> of the formula: ";
    }
...
function generateHeadersOfTheExplanation(explanation, isLanguageChanging) {
    ...
    if (!isNegationChecked) {
        if (whichLanguage === 'sk') {
            ...
            explanation.descCNF.innerHTML = "<strong>2. step is</strong> to convert the formula to <strong>CNF</strong>: ";
        }
    } else {
        if (whichLanguage === 'sk') {
            explanation.descCNF.innerHTML = "<strong>1. krok</strong> je <strong>negácia</strong> formuly: ";
        } else if (whichLanguage === 'en') {
            explanation.descCNF.innerHTML = "<strong>1. step</strong> is <strong>negation</strong> of the formula: ";
    ...
    if (!isNegationChecked) {
        if(isDIMACS) {
            if(whichLanguage === 'sk') {
                explanation.descClauses.innerHTML = "<strong>1. krok</strong> je <strong>negácia</strong> formuly: ";
            } else if(whichLanguage === 'en') {
                explanation.descClauses.innerHTML = "<strong>1. step</strong> is <strong>negation</strong> of the formula: ";
            }
        } else {
            if(whichLanguage === 'sk') {
                explanation.descClauses.innerHTML = "<strong>3. krokom</strong> je použitie <strong>pravidla rezolúcie</strong> na jednotlivé klauzuly: ";
            } else if(whichLanguage === 'en') {
                explanation.descClauses.innerHTML = "<strong>3. step</strong> is to apply <strong>the resolution rule</strong> to the individual clauses: ";
            }
        }
    } else {
        if(isDIMACS) {
            if(whichLanguage === 'sk') {
                ...
                explanation.descClauses.innerHTML = "<strong>1. step</strong> is to apply <strong>the resolution rule</strong> to the individual clauses: ";
            }
        } else {
            ...
            if(whichLanguage === 'sk') {
                ...
                explanation.descClauses.innerHTML = "<strong>2. step</strong> is to apply <strong>the resolution rule</strong> to the individual clauses: ";
            }
        }
    }
}

const toggleButton = document.getElementById('toggleResolutionButton');
const resolutionContainer = document.getElementById('resolution-container');
let isVisible = false;
...
function formulaExplanation(explanation) {
    ...
    if (!isDIMACS) {
        ...
        if (simplifiedCnfFormula === '⊤') {
            generateHeadersOfTheExplanation(explanation, false);
...
buttonSVGDynamic.addEventListener('click', function() {
...
document.getElementById('toggleVisibilityButton').addEventListener('click', function() {
    ...
    let collapse_steps = document.getElementById("collapseSteps");
    ...
    if (treeContainer.style.display === "none") {
        ...
        collapse_steps.style.display = 'flex';
